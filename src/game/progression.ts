...
export const advancedClasses:ClassDef[]=[
  ...
  {id:'bard',name:'Bard',icon:'🎵',description:'Suporte musical e dano à distância.',minBase:40,minJob:40,bonuses:{dex:5,agi:3,int:2}},
  {id:'dancer',name:'Dancer',icon:'💃',description:'Suporte musical e dano à distância.',minBase:40,minJob:40,bonuses:{dex:5,agi:3,int:2}},
...
export const advancedSkills:SkillDef[]=[
  ...
  skill('bowlingBash','Bowling Bash','knight','damage',2.8,'⚔'),skill('twoHandQuicken','Two-Hand Quicken','knight','buff',1.28,'💨'),skill('holyCross','Holy Cross','crusader','damage',2.5,'✝','Holy'),skill('guard','Guard','crusader','buff',1.20,'🛡'),
  skill('stormGust','Storm Gust','wizard','damage',3.0,'❄','Water'),skill('meteorStorm','Meteor Storm','wizard','damage',3.2,'☄','Fire'),skill('heavenDrive','Heaven Drive','sage','damage',2.7,'🪨','Earth'),skill('endow','Elemental Endow','sage','buff',1.24,'🌀'),
  skill('blitzBeat','Blitz Beat','hunter','damage',2.6,'🦅','Neutral'),skill('attentionConcentrate','Attention Concentrate','hunter','buff',1.25,'🎯'),skill('musicalStrike','Musical Strike','bard','damage',2.3,'🎵'),skill('dissonance','Dissonance','bard','buff',1.20,'🎶'),skill('throwArrow','Throw Arrow','dancer','damage',2.3,'💃'),skill('serviceForYou','Service For You','dancer','buff',1.20,'🎼'),
  skill('priestHeal','Heal','priest','heal',.35,'✚','Holy'),skill('priestHolyLight','Holy Light','priest','damage',2.5,'☀','Holy'),skill('occultImpaction','Occult Impaction','monk','damage',2.8,'👊'),skill('fury','Fury','monk','buff',1.30,'🔥'),skill('sonicBlow','Sonic Blow','assassin','damage',3.0,'🗡'),skill('enchantPoison','Enchant Poison','assassin','buff',1.25,'☠','Poison'),skill('backStab','Back Stab','rogue','damage',2.7,'🎭'),skill('swordMastery','Sword Mastery','rogue','passive',1.025,'⚔'),
  skill('cartRevolution','Cart Revolution','blacksmith','damage',2.8,'🛒'),skill('powerThrust','Power Thrust','blacksmith','buff',1.30,'🔨'),skill('acidTerror','Acid Terror','alchemist','damage',2.8,'⚗','Poison'),skill('potionPitcher','Potion Pitcher','alchemist','heal',.30,'🧪'),
  skill('spiralPierce','Spiral Pierce','lordKnight','damage',3.4,'🐉'),skill('sacrifice','Sacrifice','paladin','damage',3.3,'🩸','Holy'),skill('napalmVulcan','Napalm Vulcan','highWizard','damage',3.5,'✨','Ghost'),skill('doubleCasting','Double Casting','professor','buff',1.36,'📚'),skill('sharpShooting','Sharp Shooting','sniper','damage',3.4,'🎯'),skill('arrowVulcan','Arrow Vulcan','clown','damage',3.3,'🎶'),skill('arrowVulcanGypsy','Arrow Vulcan','gypsy','damage',3.3,'🎼'),skill('assumptio','Assumptio','highPriest','buff',1.34,'☀'),skill('asuraStrike','Asura Strike','champion','damage',4.2,'💥'),skill('soulDestroyer','Soul Destroyer','assassinCross','damage',3.6,'☠'),skill('fullStrip','Full Strip','stalker','buff',1.30,'🎩'),skill('cartTermination','Cart Termination','whitesmith','damage',3.6,'⚒'),skill('acidBomb','Acid Demonstration','creator','damage',3.8,'🧪','Neutral'),
  skill('ignitionBreak','Ignition Break','runeKnight','damage',4.0,'🔥','Fire'),skill('overbrand','Overbrand','royalGuard','damage',3.9,'🏰'),skill('crimsonRock','Crimson Rock','warlock','damage',4.1,'🌋','Fire'),skill('psychicWave','Psychic Wave','sorcerer','damage',4.0,'🌀','Ghost'),skill('arrowStorm','Arrow Storm','ranger','damage',4.0,'🏹'),skill('severeRainstorm','Severe Rainstorm','minstrel','damage',3.9,'🎻','Water'),skill('severeRainstormW','Severe Rainstorm','wanderer','damage',3.9,'🎤','Water'),skill('adoramus','Adoramus','archBishop','damage',4.0,'⛪','Holy'),skill('tigerCannon','Tiger Cannon','sura','damage',4.1,'🐯'),skill('crossRipper','Cross Ripper Slasher','guillotineCross','damage',4.2,'🦂'),skill('triangleShot','Triangle Shot','shadowChaser','damage',3.8,'🎩'),skill('axeTornado','Axe Tornado','mechanic','damage',4.0,'⚙','Wind'),skill('cartCannon','Cart Cannon','genetic','damage',4.0,'🧬'),
]
...
export function jobOptions(classId:string,baseLevel:number,jobLevel:number,rebirths:number){
  return advancedClasses.filter(cls=>{const meta:JobMeta|undefined=jobMeta[cls.id];return meta?.parent===classId&&baseLevel>=meta.minBase&&jobLevel>=meta.minJob&&rebirths>=(meta.rebirths||0)})
}
...
export function rathenaJobNames(classId:string){
  ...
  while(id){const meta:JobMeta|undefined=jobMeta[id];if(!meta)break;names.add(meta.rathena);id=meta.parent}
  return names
}

const elementTable:Record<string,Partial<Record<string,number>>>={Fire:{Earth:1.5,Undead:1.25,Water:.5,Fire:.25},Water:{Fire:1.5,Earth:.5,Water:.25,Wind:.75},Wind:{Water:1.5,Earth:.75,Wind:.25},Earth:{Wind:1.5,Fire:.75,Earth:.25},Holy:{Dark:1.5,Undead:1.5,Holy:.25},Dark:{Holy:1.5,Dark:.25,Undead:.5},Ghost:{Ghost:1.25,Neutral:.75},Poison:{Plant:1.25,Poison:.25,Undead:.5},Neutral:{Ghost:.25}}
export function elementMultiplier(attackElement:string|undefined,target:string){const atk=(attackElement||'Neutral').split(' ')[0],def=target.split(' ')[0];return elementTable[atk]?.[def]??1}

export function huntHub(map:MapDef){const level=map.monsterLevel||map.minLevel;return hubDefs.find(h=>level>=h.min&&level<=h.max&&h.biomes.includes(map.bg))?.id??(level>=100?'endgame':'prontera')}
export function huntScore(map:MapDef,baseLevel:number,attack:number,defense:number){if(baseLevel<map.minLevel)return -Infinity;const levelDelta=Math.abs(map.monsterLevel-baseLevel),survivability=Math.max(0,100-Math.max(0,map.monsterAtk-defense/5)*6),speed=Math.min(100,(attack/Math.max(1,map.monsterHp))*900),reward=Math.log10(10+map.exp+map.jobExp)*25;return reward+survivability+speed-levelDelta*3-(map.boss?18:0)}
...
export function refineChance(item:ItemDef,current:number){const weaponLevel=item.weaponLevel||1,safe=weaponLevel<=1?7:weaponLevel===2?6:weaponLevel===3?5:4;if(current<safe)return 1;return Math.max(.15,.9-(current-safe)*.12)}
